...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    const [user] = await db.query(`select * from uber_user where email='${email}'`);

    if(user.length === 0){
      return errorResponse(res,400,`your email or password is invalid !!`)
    }else{
      ...
      if(pwd){
         ...
         if(user_role === "driver"){
          url = "/uber/driver/home"
          const [driver_id] = await db.query(
                `select id from driver where user_id='${user[0].id}'`
              );
         ...
         return response(res,200,{url : url},"Logged in successfully...")
      }else{
        return errorResponse(res,400,"your email or password is invalid !!")
      }
    }
    ...
    //   // console.log("d_id",d_id[0]);
    //   if (data[0]) {

    //     res.cookie("accessToken",token);

    //     return response(res, 200, {data : data[0], url : url }, "User Login Successfully");
