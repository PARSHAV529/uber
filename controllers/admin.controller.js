    ...
    if (d1.getTime() === d2.getTime()) {
      try {
        const [customRevenue] = await db.query(
          `select DATE(created_at) as date , SUM(admin_cut) as revenue from admin_revenue where created_at >= DATE('${start_date}') and created_at < DATE_ADD('${end_date}', INTERVAL 1 DAY)  GROUP BY DATE(created_at) ORDER BY date`
        );
...
export const driverRequest = async (req, res) => {
  ...
  try {
    const [driverRequests] = await db.query(`
        select d.id as driver_id, u.first_name,u.last_name, u.email, d.document_status, d.vehicle_approved, d.created_at from uber_user as u join driver as d on u.id = d.user_id where ( d.document_status = 'pending' and d.vehicle_approved = 'pending' ) or (d.document_status = 'approved' and d.vehicle_approved = 'pending' ) or (d.document_status = 'pending' and d.vehicle_approved = 'approved' )
          `);
...
export const driverRejectedRequest = async (req, res) => {
  ...
  try {
    const [driverRejectedRequests] = await db.query(`
        select d.id as driver_id, u.first_name,u.last_name, u.email, d.document_status,d.vehicle_approved, d.created_at from uber_user as u join driver as d on u.id = d.user_id where d.document_status = 'rejected' or d.vehicle_approved = 'rejected'
          `);
...
export const approveVehical = async (req, res) => {
  ...
  try {
    const [vehicleDbResponse] = await db.query(
      `update vehicle set status = 'approved', remark = '' where id=${req.body.vehicle_id} and DID = ${req.body.driver_id}`
    );
    ...
    return response(
      ...
      200,
      {
        driverDbResponse: driverDbResponse,
        vehicleDbResponse:vehicleDbResponse,
        span_text: "approved",
      },
...
export const rejectVehical = async (req, res) => {
  ...
  try {
    const [vehicleDbResponse] = await db.query(
      `update vehicle set status = 'rejected',remark = '${req.body.remark}' where id=${req.body.vehicle_id} and DID = ${req.body.driver_id}`
    );

    const [driverDbResponse] = await db.query(
      `update driver set vehicle_approved = 'rejected' where id = ${req.body.driver_id}`
    );
    // console.log(driverDbResponse);
    // console.log(driverDbResponse);
...
let vehical_remark
  ...
  try {
    ...
    const [vehicleDbResponse] = await db.query(`select * from vehicle where id = ${req.body.vehicle_id} and DID = ${req.body.driver_id}`);
    // // console.log(vehicleDbResponse);

    if(vehicleDbResponse[0].status == 'rejected'){
      vehical_approval_status = false
      vehical_remark = vehicleDbResponse[0].remark
    }
    ...
    if (vehical_approval_status && document_approval_status) {
      ...
      mailOptions = {
        ...
        html: `<p>Dear ${req.body.driver_name},</p>
               ...
               After a thorough review, we regret to inform you that your request has been declined due to <strong>${reason} and ${vehical_remark}</strong>.</p>
      ...
      mailOptions = {
        ...
        html: `<p>Dear ${req.body.driver_name},</p>
               <p>Thank you for submitting your documents and vehicle details for verification with Uber. 
               After a thorough review, we regret to inform you that your request has been declined due to <strong>${vehical_remark}</strong>.</p>
               <p>To proceed with your registration, please ensure that:</p>
               <ul>
                   <li>Provide any missing or correct documents, if applicable.</li>
                   <li>Ensure your vehicle meets all required criteria.</li>
