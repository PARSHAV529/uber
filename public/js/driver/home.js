...
startRide.addEventListener("click", () => {
  location.href = "/uber/driver/rides";
});
...
async function getRides(ride_type) {
  ...
  dayEarning.innerHTML =
    "$" + (response.data.dailyIncome == null) ? "0" : response.data.dailyIncome;
  const completedRides = document.getElementById("completed-rides");
  completedRides.innerHTML = response.data.dailyRides;
  const activeTime = document.getElementById("working-hours");
  activeTime.textContent = toHour(response.data.activeTime[0].active_time);
}
...
function toHour(seconds) {
  ...
  return (
    date.toISOString().slice(11, 16).split(":")[0] +
    " Hr " +
    date.toISOString().slice(11, 16).split(":")[1] + " Min"
  );
}
...
async function getNotificationcnt() {
  let response = await fetch("/uber/api/driver/get-notification-count", {
    method: "POST",
    headers: {
