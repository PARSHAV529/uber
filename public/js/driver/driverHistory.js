...
async function getMonthlySummary() {
  const d_id = 1;
  const responsetxt = await fetch(`/uber/api/driver/monthly-summary/${d_id}`, {
    method: "GET",
  });

  const response = await responsetxt.json();
  console.log(response);
  document.getElementById("rideMonth").textContent =
    response.data.monthRides + " Rides this month";

  document.getElementById("earnMonth").textContent =
    "$" + response.data.monthIncome + " Earning this month";

  document.getElementById("hourMonth").textContent =
    Math.round(response.data.monthTime[0].active_time / 3600) + " Hours this month";
}
...
function renderRides(rides) {
  ...
  rides.forEach((ride) => {
    ...
    statusDiv.style.width = ride.status === "completed" ? "75px" : "70px";
...
function seeRideDetail(data) {
  ...
  <span><strong>Time : ${parseInt(
    (new Date(data.drop_time) - new Date(data.pickup_time)) / 60000
...
function convertTime(time) {
  ...
  let tomonth = monthNames[new Date(timestamp).getMonth()];
