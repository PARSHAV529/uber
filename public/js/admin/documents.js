const rejected_requests_div = document.getElementById('rejected-requests-div');
let currentPage = 1;
...
async function docVerificationPanel() {
  ...
  try {
    const response = await fetch("/uber/api/admin/driver-rejected-request", {
      method: "GET",
    });
    const result = await response.json();

    if (response.status === 200) {
      setRejectedRequests(result.data.driverRejectedRequests);
      // console.log(result);
...
function setRejectedRequests(data){
  ...
  if (data.length > 0) {
    data.forEach((driver) => {
      ...
      activity_item.innerHTML = `
        ...
        driver.last_name
            ...
            <span class="activity-status status_span">rejected</span>
            <div class="activity-time">${getUserTime(driver.created_at)}</div>
            <button onclick="viewDocs(
              ${driver.driver_id}
            )" class="activity-action">
      ...
      `;
      rejected_requests_div.appendChild(activity_item);
    });
    ...
    rejected_requests_div.innerHTML = "<p>No Rejected requests are there</p>";
  }
