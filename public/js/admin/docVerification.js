let vehicle_rejection_form = document.getElementById("vehicle-rejection-form");
...
let vehicle_status_div = document.getElementById("vehicle_status_div");
...
async function setDocuments() {
  ...
  try {
    ...
    if (resp.status === 200) {
      ...
      // // console.log(data.data.vehicalDetails);
  ...
  if (documentList.length > 0) {
    ...
    documentList.forEach((doc) => {
      ...
      driver_doc_list.innerHTML = `
            ...
            ${
              driverDetails[0].profile_photo
                ? `<img src="${driverDetails[0].profile_photo}" alt="">`
                : ""
            }
            <span class="driver-name">${driverDetails[0].first_name} ${
        driverDetails[0].last_name
      }</span>
            </div>
...
function setVehicleDocuments(vehicalDetails, driverDetails) {
  ...
  if (vehicalDetails.length > 0) {
    ...
    driver_name.forEach((name) => {
      name.innerHTML =
        driverDetails[0].first_name + " " + driverDetails[0].last_name;
    });
    ...
    vehicle_status_div.innerHTML = `${vehicalDetails[0].status}`;
    remark.innerHTML = `${vehicalDetails[0].remark}`;
    addClassesToStatus(doc_status);
    addClassesToStatus([vehicle_status_div]);

    puc = vehicalDetails[0].puc;
    rc_book = vehicalDetails[0].rc_book;
    insurance = vehicalDetails[0].insurance;

    document.getElementById("vehicle-type").value = `${vehicalDetails[0].type}`;

    document.getElementById("capacity").value = `${vehicalDetails[0].capacity}`;

    document.getElementById("colour").value = `${vehicalDetails[0].colour}`;

    document.getElementById(
      "insurance-exp-date"
    ).value = `${vehicalDetails[0].insurance_exp_date}`;

    document.getElementById(
      "number_plate"
    ).value = `${vehicalDetails[0].number_plate}`;

    document.getElementById(
      "puc-exp-date"
    ).value = `${vehicalDetails[0].puc_exp_date}`;

    document.getElementById(
      "dl-number"
    ).value = `${driverDetails[0].dl_number}`;

    document.getElementById(
      "dl-exp-date"
    ).value = `${driverDetails[0].dl_exp_date}`;

    document.getElementById("vehicle-accept").addEventListener("click", () => {
      acceptVehicle(vehicalDetails[0].DID, vehicalDetails[0].id);
    });
    ...
    document.getElementById("vehicle-reject").addEventListener("click", () => {
      rejectVehicle(vehicalDetails[0].DID, vehicalDetails[0].id);
    });
  } else {
...
async function acceptVehicle(driver_id, vehicle_id) {
  ...
  try {
    ...
    if (resp.status === 200) {
      ...
      const data = await resp.json();
      alert(data.message);
      vehicle_status_div.innerHTML = `${data.data.span_text}`;
      remark.innerHTML = "";
      addClassesToStatus([vehicle_status_div]);
...
async function rejectVehicle(driver_id, vehicle_id) {
  ...
  submit_vehicle_rejection_form.addEventListener("click", async (e) => {
    ...
    try {
      ...
      if (resp.status === 200) {
        ...
        alert(data.message);
        vehicle_status_div.innerHTML = `${data.data.span_text}`;
        remark.innerHTML = `${data.data.remark}`;
        addClassesToStatus([vehicle_status_div]);
        vehicle_rejection_form.reset();
...
async function acceptDriver(driver_id, email, name, vehicle_id) {
  try {
...
function finalSubmitVisible() {
  ...
  setTimeout(() => {
    ...
    if (
      (arr.includes("rejected") || arr.includes("approved")) &&
