...
document.getElementById("submit_btn").addEventListener("click", async () => {
  ...
  if (res.ok) {
    ...
    // console.log(data.error); document.getElementById('vehicle_insurance_exp_err').innerHTML = '';
      document.getElementById("vehicle_colour_err").innerHTML = '';
      document.getElementById("vehicle_type_err").innerHTML = '';
      document.getElementById("vehicle_number_err").innerHTML = '';
      document.getElementById('vehicle_puc_exp_err').innerHTML = '';
      document.getElementById('vehicle_insurance_exp_err').innerHTML = '';

    data.error.forEach((error) => {
      console.log(error);
     

      if (error.includes("colour")) {
        console.log("err inside colour");

        document.getElementById("vehicle_colour_err").innerHTML = error;
      }
      if (error.includes("type")) {
        // console.log('err inside type');

        document.getElementById("vehicle_type_err").innerHTML = error;
      }
      if (error.includes("number")) {
        console.log("err inside number_plate");
        document.getElementById("vehicle_number_err").innerHTML = error;
      }
      if(error.includes("PUC")){
        document.getElementById('vehicle_puc_exp_err').innerHTML = error;
      }
      if(error.includes('insurance')){
        document.getElementById('vehicle_insurance_exp_err').innerHTML = error;
      }
    });
  }
});
          ...
          document
            ...
            ?.addEventListener("click", async () => {
              ...
              if (response.ok) {
                ...
                console.log("data");
                console.log(data);
                document.getElementById(btn.id.split("_")[0]).innerHTML =
                  data.error;
                document
                  .getElementById("document")
...
fetch("/getvehicleDocumentData")
  ...
  .then(({ data }) => {
    ...
    console.log(
      data[0]?.vehicle_approved != "rejected",
      data[0]?.vehicle_approved != "undefined"
    );
    ...
    if (
      ...
      document
        .querySelectorAll(".custom-file-upload-vehicle")
        .forEach((file, i) => {
          // if (i == 0) {
          //   return;
          // }
          ...
          const insurance_Exp_date = document.getElementById(
            "insurance_Exp_date_popup"
          );
