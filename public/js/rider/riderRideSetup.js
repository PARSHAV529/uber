...
async function getCoordinates(place) {
  ...
  const response = await fetch(
    `https://maps.gomaps.pro/maps/api/geocode/json?address=${place}&key=AlzaSyoqV9Pqlz9M4gv3Tp0CaG9gymclnMytBta`
  );
...
//   try {
//     const response = await fetch(
//       `https://api.olamaps.io/places/v1/geocode?address=
//         ${address}
//       &api_key=BLNwFEGKM4Xsj1EcHTCTo0abDSIXP8aFJlRZFrKb`
//     );
//     const data = await response.json();
//     // console.log(data.geocodingResults[0].geometry.location.lat,"coords place");
...
async function fetchMap(start, end) {
  ...
  distance = (Math.round(distance) / 1000).toFixed(1);
  time = (Math.round(time) / 1000 / 60).toFixed(0);
  console.log(distance, time);
...
function initMap(source, destination) {
  ...
  new MapmyIndia.Marker({
    ...
    // icon: "https://images.ctfassets.net/3prze68gbwl1/assetglossary-17su9wok1ui0z7w/c4c4bdcdf0d0f86447d3efc450d1d081/map-marker.png",
    icon: "https://cdn-icons-png.flaticon.com/128/732/732135.png",
    draggable: false,
    // icon: {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function searchRide() {
   ...
   let pts = [];
    ...
    let sourceStr = document.getElementById("pickup-location").value;
    let destinationStr = document.getElementById("dropoff-location").value;
    const response = await fetch(
      `https://maps.gomaps.pro/maps/api/distancematrix/json?origins=${sourceStr}&destinations=${destinationStr}&key=AlzaSyVUfJ01vbIDBVpMvH_ek4bKF48121n6yOY`
    );
    ...
    let bikeAmount = Math.round(distance * 10.3);
    let autoAmount = Math.round(distance * 14);
    let carAmount = Math.round(distance * 25);
      ...
      "vehicle-cards"
              ...
              <div>
                <div style='display:flex; align-items:center; gap:5px;'>
                  <h2 style='margin:0;'>Go Sedan</h2> <span><i class="fa-solid fa-user"></i> 4</span>
                </div>
                <p style='margin:0;' class="dist">${time} mins away • <span class="time">${strTime}</span></p>
              </div>
              <div>
            ...
            <div class="cards" data-selected="false" data-type="auto">
              ...
              <div>
                <div style='display:flex; align-items:center; gap:5px;'>
                  <h2 style='margin:0;'>Auto</h2> <span><i class="fa-solid fa-user"></i> 3</span>
                </div>
                <p style='margin:0;' class="dist">${time} mins away • <span class="time">${strTime}</span></p>
              </div>
              <div>
            ...
            <div class="cards" data-selected="false" data-type="moto">
              ...
              <div>
                <div style='display:flex; align-items:center; gap:5px;'>
                  <h2 style='margin:0;'>Moto</h2> <span><i class="fa-solid fa-user"></i> 1</span>
                </div>
                <p style='margin:0;' class="dist">${time} mins away • <span class="time">${strTime}</span></p>
              </div>
              <div>
...
requestRide.addEventListener("click", async () => {
  ...
  const sourceStr = document.getElementById("pickup-location").value;
  const destinationStr = document.getElementById("dropoff-location").value;
  ...
  if (vehicleType === "moto") {
    fare_amount = Math.round(distance * 10.3);
  } else if (vehicleType === "auto") {
    fare_amount = Math.round(distance * 14);
  } else {
    fare_amount = Math.round(distance * 25);
  }

  console.log("type", vehicleType, distance, fare_amount);
  ...
  if (response.status === 201) {
    ...
    document.cookie = `trip_request_id=${data.data[0].insertId}; path=/`;

    location.href = `/uber/rider/request?source=${source.lat},${source.lng}&destination=${destination.lat},${destination.lng}&vehicletype=${vehicleType}`;
  } else {
    let data = await response.json();
